...
public class RoomList extends SubCommand{

	public RoomList(){
		super("목록", "소유한 방 목록을 확인합니다.");
	...
	public boolean execute(CommandSender sender, String[] args){
				...
				if(land.hasRoom()){
					for(Room room : land.getRooms().values()){
						if(room.isOwner(player)){
							String line = "§l§a[" + world.getCustomName() + " 월드] " + room.getAddress() + "번방 §r§7(" + Integer.toString(room.xLength()) + "x" + Integer.toString(room.zLength()) + ", 높이 : " + room.yLength() + ")";
		...
		if(information.size() < 1){
			Message.normal(player, targetName + "님은 소유중인 방이 없습니다.");
		}else{
			Message.page(player, targetName + "님의 방 목록", information, page);
		}
		return true;
	}
}